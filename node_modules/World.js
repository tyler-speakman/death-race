...
var logger = require('logger').instance('World');
...
function World(tmpDeathWall) {
  ...
  function update() {
    logger.info('World', 'update()', toString());
    if (!world.isReady) {
      return;
    }

    players.forEach(function(player,iPlayer){
      if (!player.isLiving) {
        return;
      }
  ...
  function addPlayer(player) {
    logger.info('world', 'addPlayer');
    players.push(player);
  }

  function removePlayer(playerId) {
    logger.info('world', 'removePlayer', arguments);
    logger.info('world', 'removePlayer', '..', 'searching', players.length);

    var playerIndex = _getPlayerIndex(playerId);
    players.splice(playerIndex, 1);
    logger.info('world', 'removePlayer', '..', 'matched', players.length);
  }
  ...
  function _getPlayerIndex(playerId) {
    logger.info("World", "_getPlayerIndex", playerId);
    var playerIndex = players.findIndex(function (value, key) {
      var player = value;

      return player.id === playerId;
    });
    logger.info(playerIndex);
    return playerIndex;
    // for (var iPlayer = 0; iPlayer < players.length; iPlayer++) {
